              ...
              <div class="card card-primary card-outline">
                <div class="card-body box-profile">
                  <div class="text-center">
                     <img class="profile-user-img img-fluid img-circle" src="{{ asset('dist/img/user_default.png') }}"
                        {{-- src="{{ asset(''.$data->photo.'') }}"
                         alt="{{ $data->nom }}_photo_profil" --}}>
                  </div>

                   <h3 class="profile-username text-center"> ANZIAN Méa Yannick{{--{{ strtoupper($data->nom) }} {{ $data->prenoms }}--}}</h3>

                   <p class="text-muted text-center"><i> Chef departement{{--{{ $data->role->libelle }}--}}</i></p>

                  <ul class="list-group list-group-unbordered mb-3">
                    <li class="list-group-item">
                       <b>Département </b> <a class="float-right">{{--{{ strtoupper($data->service->departement->nom) }}--}} Archivages</a>
                    </li>
                    <li class="list-group-item">
                       <b>Service </b> <a class="float-right">{{--{{ strtoupper($data->service->nom) }}--}} Archives</a>
                    </li>
                    <li class="list-group-item">
                      <b>Documents Archivés</b> <a class="float-right">{{--{{ $doc }}--}}256</a>
                    </li>
                  </ul>
                    ...
                    {{ csrf_field() }}
                      <div class="card-body">
                        ...
                        <input type="hidden" name="_method" value="PUT">
                          ...
                          {{-- {{ $data->nom }}  --}}
                          " class="form-control {{ $errors->first('nom','is-invalid')}} " name="nom" id="nom" placeholder="Enter le nom du département">
